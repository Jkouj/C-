...
void ReadFile(std::string file_name, std::vector<std::vector<int> > * grid){
 ...
 int main (int argc, char*argv[]){
    ...
    //argv 4 is X the row of the starting cell
    std::string x = argv[4];
    int X = std::stoi(x);
  
    //argv 5 is Y the column of the starting cell
    std::string y = argv[5];
    int Y = std::stoi(y);
  
    //argv 6 is CONN the connection type, can be 4 or 8
    std::string conn = argv[6];
    ...
    //rows//columns//Xcord//Ycord//CONNtype//gridVector
    int final = recurse(M,N,Y-1,X-1,CONN, grid);
